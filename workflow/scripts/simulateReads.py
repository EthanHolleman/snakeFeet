...
class readGenerator():
    ...
    def __init__(self, seq_record, conversion_eff, avg_loop_length=200, loop_length_sd=20):
...
def make_chromosomes(chromosome_table, chr_output_dir, read_output_dir, record_path):
    ...
    for index, row in chromosome_table:
        ...
        generator = chromosomeGenerator(
            row['chromosome_id'], row['nickname'], row['length'], 
            row['c_content'], row['c_spacing']
        )
